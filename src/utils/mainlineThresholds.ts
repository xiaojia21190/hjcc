 ...
 *
 * 标定结论：该口径方向为正（mainline 组龙头此后 20 日平均 +3.09%、跑赢率 52.3%，
 * 而 rotation 组 -3.92%、24.3%），且三项门槛越高收益越单调改善。
 * 但**刻意不上调阈值**：可用行情仅 600 个交易日，相邻观察点共享 19/20 的窗口，
 * 44 个 mainline 样本折算下来只有约 2 个独立事件；在这种样本上按收益调参
 * 几乎必然过拟合。前视窗口敏感性也不稳定（40 日 +4.32%、60 日 -5.91%、
 * 120 日 +3.68%），不具备可依赖的形状。等历史积累更长后再重跑标定。
 */
